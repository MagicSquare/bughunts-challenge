...
function Game(hashTag, mapGame, actors) {
    ...
    this.initialMap = mapGame;

    this.initialActors = actors;
    this.restart();
}
...
Game.prototype.restart = function restart() {
    ...
    for(var i = 0; i < this.initialMap.length; ++i) {
        this.map.push(this.initialMap[i].slice());
    }
    this.actors = clone(this.initialActors);
};
...
Game.prototype.moveBugForward = function moveBugForward(nbMove) {
...
Game.prototype.moveBugBackward = function moveBugBackward(nbMove) {
    this.moveBugForward(-nbMove);
};

Game.prototype.turnBugLeft = function turnBugLeft(nbMove) {
...
Game.prototype.turnBugRight = function turnBugRight(nbMove) {
    for (var i = 0; i < nbMove; i++) {
        this.bug.dir.invert().mul(-1, 1); // Equivalent to the 2*2 rotation matrix
